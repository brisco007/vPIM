...
int main(int argc, char** argv) {
    ...
    DPU_FOREACH (dpu_set, dpu) {
        ...
        if(dpuNumRows > 0) {
            uint32_t dpuStartRowIdx = dpuIdx*numRowsPerDPU;
            //printf("DPU: %d, from Row: %d, Size: %ld\n", dpuIdx, dpuStartRowIdx, dpuNumRows*sizeof(float));
	...
	printf("Results: ");
	printf("\n\tCPU-DPU: ");
	printf("%f", loadTime*1e3);
	printf("\n\tDPU: ");
	printf("%f", dpuTime*1e3);
	printf("\n\tDPU-CPU: ");
	printf("%f", retrieveTime*1e3);
    ...
    for(uint32_t rowIdx = 0; rowIdx < numRows; ++rowIdx) {
        ...
        if(diff > tolerance || diff < -tolerance) {
            PRINT_ERROR("Mismatch at index %u (CPU result = %f, DPU result = %f) at DPU %d", rowIdx, outVectorReference[rowIdx], outVector[rowIdx], rowIdx/numRowsPerDPU);
        }
    }
