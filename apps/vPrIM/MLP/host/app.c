	...
	for (unsigned int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		for(int lay = 1; lay < NUM_LAYERS; lay++){
			...
			if(rep >= p.n_warmup)
				stop(&timer, 4);
			if(lay<NUM_LAYERS-1){
				printf("%ld, %ld, %ld, ", max_rows_per_dpu * sizeof(T), n_size_pad * sizeof(T), max_rows_per_dpu * n_size_pad * sizeof(T));
			}
			else{
				printf("%ld, %ld, %ld\n", max_rows_per_dpu * sizeof(T), n_size_pad * sizeof(T), max_rows_per_dpu * n_size_pad * sizeof(T));
			}
			if (rep >= p.n_warmup)
			{
				start(&timer, 2, rep - p.n_warmup);
