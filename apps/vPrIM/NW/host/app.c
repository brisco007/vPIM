...
int main(int argc, char **argv) {
    ...
    for (unsigned int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        for (unsigned int blk = 1; blk <= (max_cols-1)/BL; blk++) {
            ...
            start(&timer, 5, rep - p.n_warmup + blk - 1);
            int element_per_dpu = (BL+2)*(BL+1)*blocks_per_dpu;
            int32_t **buffer_array = malloc(nr_of_dpus*sizeof(int32_t *));
            for(int k=0;k<nr_of_dpus;k++){
                buffer_array[k] = malloc(element_per_dpu*sizeof(int32_t));
            }
            int index_offset = 0;
                    ...
                    DPU_FOREACH(dpu_set, dpu, i) {
                        ...
                        if(bl== 0){
                            for(int k=0; k<(BL+2);k++){
                                buffer_array[i][index_offset+k] = dpu_pointer[input_itemsets_offset+k];
                            }
                        } else {
                            buffer_array[i][index_offset] = dpu_pointer[input_itemsets_offset];
                            buffer_array[i][index_offset+1] = dpu_pointer[input_itemsets_offset+1];
                        } 
            ...
            DPU_FOREACH(dpu_set,dpu,i) {
                DPU_ASSERT(dpu_prepare_xfer(dpu, buffer_array[i]));
            }
            DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, element_per_dpu*sizeof(int32_t), DPU_XFER_DEFAULT));
            free(buffer_array);
            ...
            element_per_dpu = (BL)*BL*blocks_per_dpu;
            buffer_array = (int32_t **) malloc(nr_of_dpus*sizeof(int32_t *));
            for(int k=0;k<nr_of_dpus;k++){
                buffer_array[k] = malloc(element_per_dpu*sizeof(int32_t));
            }
            index_offset = 0;
            ...
            for (unsigned int bl_indx = 0; bl_indx < blocks_per_dpu; bl_indx++) {
                for (unsigned int bl = 0; bl < BL; bl++) {
                    ...
                    DPU_FOREACH(dpu_set, dpu, i) {
                        ...
                        for(int k=0; k<BL;k++){
                             buffer_array[i][index_offset+k] = dpu_pointer[reference_offset+k];
                        }
                    }
            ...
            DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, blocks_per_dpu * (BL+1) * (BL+2) * sizeof(int32_t), element_per_dpu*sizeof(int32_t), DPU_XFER_DEFAULT));
            free(buffer_array);
            ...
            mram_offset = 0;
            element_per_dpu = (BL+2)*(BL+1)*blocks_per_dpu;
            buffer_array = malloc(nr_of_dpus*sizeof(int32_t *));
            for(int k=0;k<nr_of_dpus;k++){
                buffer_array[k] = malloc(element_per_dpu*sizeof(int32_t));
            }
            index_offset = 0;
            ...
            DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, element_per_dpu*sizeof(int32_t), DPU_XFER_DEFAULT));
                ...
                for (unsigned int bl = 0; bl < BL + 1; bl++) {
                    ...
                    DPU_FOREACH(dpu_set, dpu, i) {
                        ...
                        if (bl == 0) // Skip the first row of the block
                            continue;
                        for(int k=0; k<(BL+2);k++){
                            dpu_pointer[input_itemsets_offset+k] = buffer_array[i][index_offset+k];
                        }
