...
*/
            ...
            int element_per_dpu = n;
            T **buffer_array = malloc(active_dpus*sizeof(T *));
           
             for(int k=0;k<active_dpus;k++){
                buffer_array[k] = malloc(element_per_dpu*M_ * m*sizeof(T));
            }
            for (unsigned int j = 0; j < M_ * m; j++ ) {
                 unsigned int i = 0;
                DPU_FOREACH(dpu_set, dpu) {
                    memcpy( buffer_array[i]+j*element_per_dpu,&A_backup[j * N_ * n + n * (i + curr_dpu)], element_per_dpu*sizeof(T));
                        i++;
                }
            ...
            DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, element_per_dpu*sizeof(T)*M_ * m, DPU_XFER_DEFAULT));
            free(buffer_array);
