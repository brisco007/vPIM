...
applications = {"VA"       : ["NR_DPUS=X NR_TASKLETS=Y BL=Z make all", "./bin/host_code -w 0 -e 1 -i 167772160 -x 1"], 
                ...
                "TRNS_old"     : ["NR_DPUS=X NR_TASKLETS=Y make all", "./bin/host_code -w 0 -e 1 -p 2048 -o 12288 -x 1"],
                "NW_old"       : ["NR_DPUS=X NR_TASKLETS=Y BL=40 BL_IN=2 make all", "./bin/nw_host -w 0 -e 1 -n 65536"],
            ...
            for r in NR_DPUS:
                for t in NR_TASKLETS:
                    for b in BL:
                        ...
                        if write_to_file:
                            if is_native: 
                                r_cmd = r_cmd +  " > "+ rootdir+"/full_results/native/"+app_name+"_"+str(r)
                            else:
                                r_cmd = r_cmd +  " > "+ rootdir+"/full_results/vPIM/"+app_name+"_"+str(r)
