...
use crate::request::vpim::{/* parse_get_vpim, */ parse_patch_vpim, parse_put_vpim};
...
impl ParsedRequest {
    ...
    pub(crate) fn try_from_request(request: &Request) -> Result<ParsedRequest, Error> {
        ...
        match (request.method(), path, request.body.as_ref()) {
            ...
            //(Method::Get, "vpim", None) => parse_get_vpim(), 
            //TODO add get_vpim if needed, REMOVE ALSO
            ...
            (Method::Put, "vpim",Some(body)) => parse_put_vpim(body),
            ...
            (Method::Patch, "vpim",  Some(body)) => parse_patch_vpim(body),
    ...
    pub(crate) fn convert_to_response(
        ...
        match request_outcome {
            Ok(vmm_data) => match vmm_data {
                ...
                VmmData::VPIMDeviceConfig(vpim_config) => {
                    Self::success_response_with_data(vpim_config)
                }
    ...
    fn test_convert_to_response() {
        let verify_ok_response_with = |vmm_data: VmmData| {
            ...
            let expected_response = match data.as_ref().unwrap() {
                ...
                VmmData::VPIMDeviceConfig(_) => todo!(),
            };
