...
use devices::virtio::{
    Balloon, BalloonConfig, VPIMDevice, VPIMDeviceConfig, BalloonStats, Block, MmioTransport, Net, BALLOON_DEV_ID, VPIM_DEV_ID, TYPE_VPIM, TYPE_BALLOON,
    TYPE_BLOCK, TYPE_NET,
};
...
impl Vmm {
    ...
    pub fn balloon_config(&self) -> std::result::Result<BalloonConfig, BalloonError> {
     ...
     /// Returns a reference to the vpim device if present.
     pub fn vpim_config(&self) -> std::result::Result<VPIMDeviceConfig, VPIMDeviceError> {
        if let Some(busdev) = self.get_bus_device(DeviceType::Virtio(TYPE_VPIM), VPIM_DEV_ID)
        {
            let virtio_device = busdev
                .lock()
                .expect("Poisoned lock")
            ...
            let config = virtio_device
                ...
                .expect("Poisoned lock")
                .as_mut_any()
                .downcast_mut::<VPIMDevice>()
                .unwrap()
                .config();

            Ok(config)
        } else {
            Err(VPIMDeviceError::DeviceNotFound)
        }
    }
    ...
    ///Update the vpim configuration
    pub fn update_vpim_config(
        &mut self,
        _mode: u64,
    ) -> std::result::Result<(), VPIMDeviceError> {
        if let Some(busdev) = self.get_bus_device(DeviceType::Virtio(TYPE_VPIM), VPIM_DEV_ID)
        {
            {
                let _virtio_device = busdev
                    .lock()
                    .expect("Poisoned lock")
                    .as_any()
                    .downcast_ref::<MmioTransport>()
