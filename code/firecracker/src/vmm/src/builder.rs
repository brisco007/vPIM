...
use devices::virtio::{Balloon,VPIMDevice, Block, MmioTransport, Net, VirtioDevice, Vsock, VsockUnixBackend};
...
pub fn build_microvm_for_boot(
    ...
    attach_vpim_devices(&mut vmm, &mut boot_cmdline, vm_resources.vpim.list.iter(), event_manager)?;
    //This should be done when it will come to deal with all the ranks
...
fn attach_vpim_devices<'a>(
    vmm: &mut Vmm,
    cmdline: &mut LoaderKernelCmdline,
    vpim_devs: impl Iterator<Item = &'a Arc<Mutex<VPIMDevice>>>,
    event_manager: &mut EventManager,
) -> std::result::Result<(), StartMicrovmError> {
    for vpim_dev in vpim_devs {
       ...
       /*  thread::spawn(move || {
        ...
        //println!("Nothing");
        let id = {
            let mut locked = vpim_dev.lock().expect("Poisoned lock");
            locked.config();
            //TODO can add the manager there ?
            locked.get_id().clone().to_string()
        };

        // The device mutex mustn't be locked here otherwise it will deadlock.
        attach_virtio_device(event_manager, vmm, id, vpim_dev.clone(), cmdline)?;
