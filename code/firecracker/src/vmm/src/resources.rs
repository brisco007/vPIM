...
pub enum Error {
    ...
    ///VPIMDevice configuration error.
    VpimDevice(VPIMDeviceConfigError),
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Error::VpimDevice(e) => write!(f, "VPIM device error: {}", e),
...
pub struct VmmConfig {
    ...
    #[serde(rename = "vpim")]
    vpim_devices: Vec<VPIMDeviceConfigFromAPI>,
    #[serde(rename = "drives")]
...
impl VmResources {
    ...
    pub fn from_json(
       ...
       /*  if let Some(vpim_config) = vmm_config.vpim_device {
            resources
                .set_vpim_device(vpim_config)
                .map_err(Error::VpimDevice)?;
        } */
        for vpim_config in vmm_config.vpim_devices.into_iter() {
            resources
                .set_vpim_device(vpim_config)
                .map_err(Error::VpimDevice)?;
        }
    ...
    pub fn update_from_restored_device(&mut self, device: SharedDeviceType) {
        match device {
            ...
            SharedDeviceType::SharedVPIMDevice(vpim) => {
                self.vpim.add_device(vpim);
            }
    ...
    pub fn update_vm_config(&mut self, machine_config: &VmUpdateConfig) -> Result<VmConfigError> {
        ...
        /* // The VM cannot have a memory size smaller than the target size
        // of the vpim device, if present.
        if self.vpim.get().is_some()
            && mem_size_mib
                < self
                    .vpim
                    .get_config()
                    .map_err(|_| VmConfigError::InvalidVmStateVPIM)?
                    .size as usize
        {
            return Err(VmConfigError::VPIMDeviceError);
        } */
    ...
    ///Set a vpim device to be attached when the VM starts.
    pub fn set_vpim_device(
        &mut self,
        config: VPIMDeviceConfigFromAPI,
    ) -> Result<VPIMDeviceConfigError> {
        self.vpim.insert(config)
    }
...
impl From<&VmResources> for VmmConfig {
    fn from(resources: &VmResources) -> Self {
        ...
        VmmConfig {
            ...
            vpim_devices: resources.vpim.configs(),
        }
    }
}
