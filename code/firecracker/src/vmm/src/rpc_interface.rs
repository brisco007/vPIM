...
//TODO investigate on t usage of this file in particular but in genreal create the architecture of 
//the device and list each file wħere we needed to implement something and why
...
use crate::vmm_config::vpim::{
    VPIMDeviceConfigError, VPIMDeviceConfigFromAPI, VPIMDeviceUpdateConfig,
};
...
pub enum VmmAction {
    ...
    GetBalloonConfig,
    // Get the vpim device configuration.
    //GetVPIMDeviceConfig,
    ...
    /// Add the vpim device or update the one that already exists using the
    /// `VPIMDeviceConfig` as input. This action can only be called before the microVM
    /// has booted.
    InsertVPIMDevice(VPIMDeviceConfigFromAPI),
    ...
    /// Update the vpim device, after microVM start.
    UpdateVPIMDevice(VPIMDeviceUpdateConfig),
...
pub enum VmmActionError {
    ...
    /// The action `SetVPIMDevice` failed because of bad user input.
    VPIMDeviceConfig(VPIMDeviceConfigError),
    ...
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        write!(
            ...
            match self {
                ...
                VPIMDeviceConfig(err) => err.to_string(),
...
pub enum VmmData {
    ...
    /// The vpim device configuration.
    VPIMDeviceConfig(VPIMDeviceConfigFromAPI),
...
impl<'a> PrebootApiController<'a> {
    ...
    pub fn handle_preboot_request(&mut self, request: VmmAction) -> ActionResult {
        ...
        match request {
            ...
            //GetVPIMDeviceConfig => self.vpim_config(),
            GetFullVmConfig => {
            ...
            InsertVPIMDevice(config) => self.insert_vpim_device(config),
    ...
    fn balloon_config(&mut self) -> ActionResult {
     ...
     /* fn vpim_config(&mut self) -> ActionResult {
        self.vm_resources
            .vpim
            .get_config() //TODO
            .map(VmmData::VPIMDeviceConfig)
            .map_err(VmmActionError::VPIMDeviceConfig)
    }  */
    ...
    fn insert_vpim_device(&mut self, cfg: VPIMDeviceConfigFromAPI) -> ActionResult {
        self.boot_path = true;
        self.vm_resources
            .set_vpim_device(cfg)
            .map(|()| VmmData::Empty)
...
impl RuntimeApiController {
    ...
    pub fn handle_request(&mut self, request: VmmAction) -> ActionResult {
        ...
        match request {
          ...
          /*   GetVPIMDeviceConfig => self
                .vmm
                .lock()
                .expect("Poisoned lock")
                .vpim_config()
                .map(|state| VmmData::VPIMDeviceConfig(VPIMDeviceConfigFromAPI::from(state)))
                .map_err(|e| VmmActionError::VPIMDeviceConfig(VPIMDeviceConfigError::from(e))),
             */ //Uncomment if needed , if we need to get a VPIM config
            ...
            UpdateVPIMDevice(new_cfg) => self.update_vpim_device(new_cfg),
    ...
    fn update_block_device(&mut self, new_cfg: BlockDeviceUpdateConfig) -> ActionResult {
     ...
     /// Updates vpim device properties (we don't have properties for the moment)
    fn update_vpim_device(&mut self, _new_cfg: VPIMDeviceUpdateConfig) -> ActionResult {
        //TODO do like the function above 
        self.vmm.lock().expect("Poisoned lock");
        println!("VPIM updated");
        Ok(VmmData::Empty)
    }
...
mod tests {
    ...
    use crate::vmm_config::vpim::VPIMDeviceBuilder;
    ...
    pub struct MockVmRes {
        ...
        pub vpim: VPIMDeviceBuilder,
        pub vsock: VsockBuilder,
        balloon_config_called: bool,
        vpim_config_called: bool,
        balloon_set: bool,
        boot_cfg_set: bool,
        vpim_set: bool,
        block_set: bool,
    ...
    impl MockVmRes {
        ...
        pub fn vpim_config(&mut self) -> Result<VPIMDeviceConfigFromAPI, VPIMDeviceConfigError> {
            if self.force_errors {
                return Err(VPIMDeviceConfigError::DeviceNotFound);
            }
            self.vpim_config_called = true;
            Ok(VPIMDeviceConfigFromAPI::default())
        }
        ...
        pub fn set_vpim_device(
            &mut self,
            _: VPIMDeviceConfigFromAPI,
        ) -> Result<(), VPIMDeviceConfigError> {
            if self.force_errors {
                return Err(VPIMDeviceConfigError::DeviceNotFound);
            }
            self.vpim_set = true;
    ...
    pub struct MockVmm {
        ...
        pub update_vpim_config_called: bool,
    ...
    impl MockVmm {
        ...
        pub fn update_vpim_config(&mut self, _: u32) -> Result<(), VPIMDeviceConfigError> {
            if self.force_errors {
                return Err(VPIMDeviceConfigError::DeviceNotFound);
            }
            self.update_vpim_config_called = true;
            Ok(())
        }
