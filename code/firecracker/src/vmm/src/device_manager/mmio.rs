...
use devices::virtio::{
    Balloon,VPIMDevice, Block, MmioTransport, Net, VirtioDevice, TYPE_BALLOON, TYPE_VPIM, TYPE_BLOCK, TYPE_NET,
    TYPE_VSOCK,
};
    ...
    pub fn kick_devices(&self) {
        ...
        let _: Result<()> = self.for_each_virtio_device(|virtio_type, id, _info, dev| {
            ...
            match virtio_type {
                ...
                TYPE_VPIM => {
                    let vpim = virtio.as_mut_any().downcast_mut::<VPIMDevice>().unwrap();
                    // If device is activated, kick the vpim queue(s) to make up for any
                    // pending or in-flight events we may have not captured in snapshot.
                    if vpim.is_activated() {
                        info!("kick vpim {}.", id);
