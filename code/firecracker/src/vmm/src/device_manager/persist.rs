...
use devices::virtio::vpim::{VPIMDevice, Error as VPIMDeviceError};
use devices::virtio::vpim::persist::{VPIMDeviceConstructorArgs, VPIMDeviceState};
...
use devices::virtio::{
    MmioTransport, VirtioDevice, TYPE_BALLOON, TYPE_VPIM, TYPE_BLOCK, TYPE_NET, TYPE_VSOCK,
};
...
#[derive(Clone, Versionize)]
/// Holds the state of a vpim device connected to the MMIO space.
// NOTICE: Any changes to this structure require a snapshot version bump.
pub struct ConnectedVPIMDeviceState {
    /// Device identifier.
    pub device_id: String,
    /// Device state.
    pub device_state: VPIMDeviceState,
    /// Mmio transport state.
    pub transport_state: MmioTransportState,
    /// VmmResources.
    pub mmio_slot: MMIODeviceInfo,
...
pub struct DeviceStates {
    ...
    /// VPIM device states.
    /// TODO: This should be a Vec<ConnectedVPIMDeviceState> once we will add multiple VPIM devices.
    #[version(start = 4, ser_fn = "vpim_serialize")]
    pub vpim_devices: Option<ConnectedVPIMDeviceState>,
}
...
pub enum SharedDeviceType {
    ...
    SharedVPIMDevice(Arc<Mutex<VPIMDevice>>),
...
impl DeviceStates {
    ...
    fn vpim_serialize(&mut self, target_version: u16) -> VersionizeResult<()> {
        if target_version < 4 && self.vpim_devices.is_some() {
            return Err(VersionizeError::Semantic(
                "Target version does not implement the virtio-PIM device.".to_owned(),
            ));
        }
...
impl<'a> Persist<'a> for MMIODeviceManager {
    ...
    fn save(&self) -> Self::State {
        let mut states = DeviceStates {
            ...
            mmds_version: None,
            ///TODO add Vec::new() for vpim_devices
            vpim_devices: None
        };
        let _: Result<(), ()> = self.for_each_device(|devtype, devid, devinfo, bus_dev| {
            ...
            match locked_device.device_type() {
                ...
                TYPE_VPIM => {
                    let vpim_state = locked_device
                        .as_any()
                        .downcast_ref::<VPIMDevice>()
                        .unwrap()
                        .save();
                    states.vpim_devices = Some(ConnectedVPIMDeviceState {
                        device_id: devid.clone(),
                        device_state: vpim_state,
                        transport_state,
                        mmio_slot: devinfo.clone(),
                    });
                }
    ...
    fn restore(
        ...
        if let Some(vpim_state) = &state.vpim_devices {
            let device = Arc::new(Mutex::new(
                VPIMDevice::restore(
                    VPIMDeviceConstructorArgs { mem: mem.clone() },
                    &vpim_state.device_state,
                )
                .map_err(Error::VPIMDevice)?,
            ));

            (constructor_args.for_each_restored_device)(
                constructor_args.vm_resources,
                SharedDeviceType::SharedVPIMDevice(device.clone()),
            );

            restore_helper(
                device.clone(),
                device,
                &vpim_state.device_id,
                &vpim_state.transport_state,
                &vpim_state.mmio_slot,
                constructor_args.event_manager,
            )?;
        }
