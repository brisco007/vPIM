...
pub enum VmConfigError {
    ...
    InvalidVmState,
    ///Same as InvalidVMState, but for the vpim device.
    InvalidVmStateVPIM,
    ///VPIM related error
...
impl fmt::Display for VmConfigError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            InvalidVmStateVPIM => write!(
                f,
                "Could not get the configuration of the previously \
                 installed vpim device to validate the memory size.",
            ),
            VPIMDeviceError => write!(
                f,
                "The user made a request on an inexistent vpim device. or something else happened \
                like the size of the virtual machine is too small for the vpim device to fit in.",
            ),
        }
    }
}
